...
export default function CollectionsPage() {
    ...
    // টোকেন রিকোয়েস্ট করার Helper ফাংশন
    ...
    const handleSave = async () => {
        try {
            const payload = { ...formData, category: selectedCategory, price: Number(formData.price) };
            const authConfig = await getAuthHeaders(); // টোকেনসহ হেডার

            if (editProduct) {
                await axios.put(`http://localhost:5000/api/products/${editProduct._id}`, payload, authConfig);
            } else {
                await axios.post("http://localhost:5000/api/products", payload, authConfig);
            }
            await fetchProducts();
            setIsOpen(false);
        } catch (error) {
        ...
        <div className="p-8">
            {categories.map((cat) => (
                <section key={cat} className="mb-15">
                    <h2 className="text-3xl font-extrabold text-accent tracking-tight mb-8 uppercase font-serif border-b border-accent/20 pb-2">{cat}</h2>
                    <div className="grid grid-cols-4 gap-6">
                        ...
                        <div onClick={() => {
                            ...
                            setEditProduct(null);
                            setFormData({ name: "", description: "", price: 0, imageUrl: "", metalType: "", gemstones: "", finish: "", ringSize: "", closureType: "" });
                            setIsOpen(true);
                        }}
            ...
            {isOpen && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
                    <div className="bg-white p-6 rounded-2xl w-[400px] shadow-xl">
                        ...
                        {selectedCategory === "Ring" && <input className="w-full border p-2 mb-2 rounded" placeholder="Ring Size" value={formData.ringSize} onChange={(e) => setFormData({ ...formData, ringSize: e.target.value })} />}
                        {selectedCategory === "Earing" && <input className="w-full border p-2 mb-2 rounded" placeholder="Closure Type" value={formData.closureType} onChange={(e) => setFormData({ ...formData, closureType: e.target.value })} />}
